...
class ConvAE_mini(chainer.Chain):
    def __init__(self, input_size, input_size2, channel_size=3, n_filters=10, n_latent=20, filter_size=3, activation='relu'):
        ...
        with self.init_scope():
            # encoder
            self.conv1 = L.Convolution2D(channel_size, n_filters, filter_size, pad=1)
            self.lenc1 = L.Linear(n_filters*self.dim1*self.dim2, n_latent)
            # decoder
            self.ldec1 = L.Linear(n_latent, n_filters*self.dim1*self.dim2)
            self.deconv1 = L.Deconvolution2D(n_filters, channel_size, filter_size, pad=1)
